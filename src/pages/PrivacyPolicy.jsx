      ...
      .then(text => {
        ...
        const htmlContent = text
          ...
          .replace(/^(.+)$/gm, (match, line) => {
            if (line.trim() === '' || line.startsWith('<') || line.startsWith('-') || line.startsWith('*')) return line;
          ...
          .replace(/<a href="([^"]+)"[^>]*>([^<]+)<\/a>/g, '<a href="$1" class="text-blue-400 hover:text-blue-300 transition-colors underline" target="_blank" rel="noopener noreferrer">$2</a>')
          .replace(/\[([^\]]+)\]\(#([^)]+)\)/g, '<a href="#$2" class="text-blue-400 hover:text-blue-300 transition-colors">$1</a>')
          .replace(/\| (.+) \|/g, '<td class="border border-zinc-600 px-4 py-2 text-zinc-300">$1</td>')
          .replace(/<td[^>]*>.*<\/td>/g, (match) => `<tr>${match}</tr>`)
          .replace(/(<tr>.*<\/tr>\s*)+/g, '<table class="w-full border-collapse border border-zinc-600 mb-6">$&</table>');
